...
const SelectUserDialog = ({ selectedFile, onClose, onPrev }: { selectedFile: any, onClose: any, onPrev: any }) => {
	...
	const handleSendMessage = async () => {
		...
		try {
			await sendMessageAction(selectedUser?._id as any, selectedFile, "image");
			...
			>
				<DialogHeader>
					...
					{users.map((user: IUserDocument) => (
						<div key={user._id as any} className='flex flex-col max-h-48 bg-sigSurface rounded-md overflow-auto'>
